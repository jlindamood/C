...
/* The function Handle_Votes is used in Add_Votes and Remove_Votes to compute the total votes in the file. *
 *  It pretty much compute our total votes as the previous assignement                                      */
...
char* Handle_Votes(char* votes_file, char* requestfile){
		...
		if (inFD ==NULL) {
			printf("Cannot find %s file in this directory.\n", votes_file);	// Satisfies output specification #1
			exit(0);
		}
...
/* the read_Req read the input.req file and parse it
 *  Each command line is handline individually     */
...
void read_Req(int sock, char* requestfile){
	...
	if (fd == NULL) {
		...
		if (strcmp(request_Name,"Open_Polls")== 0){
			...
			sprintf(buf,"%s;%s;","OP",Region_name);
		...
		else if(strcmp(request_Name, "Add_Region")==0){
			char buf[256];
			memset(buf, '0', sizeof(buf));
			trimwhitespace(requestLine[1]);
			char Region_name [15];
			...
			trimwhitespace(requestLine[2]);
			char Region_data [15];
			strcpy(Region_data,requestLine[2]);
			int RegionNamelen = strlen(requestLine[2]);
			for (int i = 0; i < (15 -RegionNamelen); i++) {
				strcat(Region_data, " ");
			}
			sprintf(buf,"%s;%s;%s","AR",Region_name,Region_data);
