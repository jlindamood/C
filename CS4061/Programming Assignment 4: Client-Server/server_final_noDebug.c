/*login: LINDA140, Bogni001
date: 04/30/18
name: JAMES_LINDAMOOD
id: LINDA140 , Michelle Bognin*/
...
return_t* add_region (node_t* nodes, char* nodename, char* dataString) {
	return_t* result = malloc(sizeof(return_t));
	...
	node_t* newNodeExists = findnode(&nodes[1], dataString);
	if (newNodeExists != NULL) {
		strcpy(result->code, "UE");
		return result;
	}
	...
	struct node* newNode = (struct node*)malloc(sizeof(struct node));
	strcpy(newNode->name, dataString);
	strcpy(newNode->name, trimwhitespace(newNode->name));
	strcpy(newNode->parentName, nodename);
	newNode->closed = -1;
	newNode->num_children = 0;
	newNode->id = i;
	nodes[i] = *newNode;

	currentNode->childNodes[currentNode->num_children] = newNode;
	currentNode->children[currentNode->num_children] = newNode->id;
...
if (strcmp(request[0], "RW") == 0){
	printf("Request received from client at %s:%d, %s %s\n", cq->clientAddress, cq->port, trimwhitespace(request[0]),request[1]);
}
else{
printf("Request received from client at %s:%d %s %s %s\n", cq->clientAddress, cq->port, trimwhitespace(request[0]),trimwhitespace(request[1]),request[2]);
}
return_t* result = malloc(sizeof(return_t));
char message[1024];
...
if (strcmp(request[0], "RW") == 0) {
	...
	} else if (strcmp(result->code, "RR") == 0) {
		strcat(message, result->result);
	}	else {
	...
	if (strcmp(result->code, "NL") == 0 || strcmp(result->code, "RC") == 0) {
	 ...
	 else if (strcmp(result->code, "IS") == 0) {
		strcat(message, result->data);
	} else if (strcmp(result->code, "RR") == 0) {
		strcat(message, result->result);
	} else {
...
else if (strcmp(request[0], "AR") == 0) {
	trimwhitespace(request[1]);
	char* nodename = request[1];
	char data[1024];
	strcpy(data, request[2]);
	result = add_region (cq->nodes, nodename, data);
	strcpy(message, result->code);
	strcat(message, ";");
	if (strcmp(result->code, "RR") == 0) {
		strcat(message, result->result);
	} else if (strcmp(result->code, "UE") == 0) {
		strcat(message, "Node to add already exists.");
	}	else {
		strcat(message, "Region was added to the graph.");
	}
	printgraph(&(cq->nodes[1]),8);
}

else {
	sprintf(message, "UC;%s;", request[0]);
}

int i = 0;
char** messageParts = (char**)malloc(2*sizeof(char*));
for ( i = 0; i < 2; i++) {
	messageParts[i] = (char**)malloc(1024*sizeof(char*));
}

makeargv(message, ";", &messageParts);


message[strlen(message)] = '\0';
int n=	send (cltsock, message, strlen(message), 0 );
printf("Sending response to client at %s:%d, %s %s\n",cq->clientAddress, cq->port, messageParts[0], messageParts[1]);
